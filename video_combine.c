...
static int open_output_file(const char *filename)
{
    ...
    printf("open_output_file(ifmt_ctx nb_streams: %d) 2\n", ifmt_ctx[0]->nb_streams);
        ...
        if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO
            ...
            * streams easily using filters */
            ...
            if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
                ...
                printf("avformat_new_stream 4 3\n");
                /* video time_base can be set to whatever is handy and supported by encoder */
                enc_ctx->time_base.num = 1;
                enc_ctx->time_base.den = 25;
                enc_ctx->gop_size = 250;
                enc_ctx->max_b_frames = 3;
                printf("avformat_new_stream 4 4\n");
                AVDictionary * d = NULL;
                ...
                printf("avformat_new_stream 4 5\n");
                av_dict_set(&d, k, v, AV_DICT_DONT_STRDUP_KEY | AV_DICT_DONT_STRDUP_VAL);
                printf("avformat_new_stream 5\n");
                ret = avcodec_open2(enc_ctx, encoder, &d);
                printf("avformat_new_stream 6\n");
            }
            else {
    ...
    printf("av_dump_format start 1\n");
    av_dump_format(ofmt_ctx, 0, filename, 1);
    printf("av_dump_format start 2\n");
...
int videocombine(GlobalContext* video_ctx)
{
    ...
    printf("open_output_file start\n");
    if ((ret = open_output_file(global_ctx->outfilename)) < 0){
        printf("open_output_file error\n");
        goto end;
    }
    printf("open_output_file success\n");

    printf("init_spec_filter start\n");
    if ((ret = init_spec_filter()) < 0)
        goto end;
    printf("init_spec_filter success\n");
